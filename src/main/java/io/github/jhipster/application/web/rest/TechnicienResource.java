...
public class TechnicienResource {
    ...
    @Timed
    public List<Technicien> getAllTechniciens(@RequestParam(required = false) String filter) {
        if ("adresse-is-null".equals(filter)) {
            log.debug("REST request to get all Techniciens where adresse is null");
            return technicienService.findAllWhereAdresseIsNull();
        }
