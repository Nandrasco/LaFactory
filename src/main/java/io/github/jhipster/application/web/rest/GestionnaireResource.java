...
public class GestionnaireResource {
    ...
    @Timed
    public List<Gestionnaire> getAllGestionnaires(@RequestParam(required = false) String filter) {
        if ("adresse-is-null".equals(filter)) {
            log.debug("REST request to get all Gestionnaires where adresse is null");
            return gestionnaireService.findAllWhereAdresseIsNull();
        }
